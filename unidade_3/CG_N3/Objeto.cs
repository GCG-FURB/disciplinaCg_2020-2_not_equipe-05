...
namespace gcgcg
{
  internal abstract class Objeto
  {
    ...
    //get set do rotulo para poder printar no console
    public char Rotulo {get=> rotulo; set => rotulo = value;}

    //Necessário o filho saber quem é o pai
    private Objeto pai = null;
    public Objeto Pai { get => pai; set => pai = value; }
    ...
    public List<Objeto> ObjetosLista { get => objetosLista; set => objetosLista = value; }
    ...
    //Método que remove os filhos (utilizado quando o objeto pai é deletado)
    public void FilhoRemoverTodos()
    {
      
      //Enquanto tiver filhos
      while(objetosLista.Count > 0){
        //Remove os filhos do filho
        this.objetosLista[0].FilhoRemoverTodos();
        //Remove o filho
        objetosLista.RemoveAt(0);
      }
